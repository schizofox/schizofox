...
in
  {
    ...
    # Global Javascript toggle. This defaults to true to retain compatibility
    # with most webpages users visit, however, this is also a security flaw in
    # the sense that Javascript can be used for fingerprint you and much more.
    # Disable if Javascript is not required.
    ...
    # enterprise policy so the setting also applies where policies are unavailable.
    "datareporting.policy.dataSubmissionEnabled" = security.telemetry.enable;
    "datareporting.healthreport.uploadEnabled" = security.telemetry.enable;
    "toolkit.telemetry.unified" = false;
    "toolkit.telemetry.enabled" = security.telemetry.enable;
    "toolkit.telemetry.archive.enabled" = security.telemetry.enable;
    "toolkit.telemetry.newProfilePing.enabled" = security.telemetry.enable;
    "toolkit.telemetry.shutdownPingSender.enabled" = security.telemetry.enable;
    "toolkit.telemetry.updatePing.enabled" = security.telemetry.enable;
    "toolkit.telemetry.bhrPing.enabled" = security.telemetry.enable;
    "toolkit.telemetry.firstShutdownPing.enabled" = security.telemetry.enable;

    # Disable Telemetry Coverage when telemetry is disabled.
    # Also see:
    #  <https://blog.mozilla.org/data/2018/08/20/effectively-measuring-search-in-firefox>
    "toolkit.telemetry.coverage.opt-out" = !security.telemetry.enable;
    "toolkit.coverage.opt-out" = !security.telemetry.enable;

    # Disable Firefox Home (Activity Stream) telemetry unless telemetry is enabled.
    ...
    "browser.safebrowsing.enabled" = security.safeBrowsing.enable;
    "browser.safebrowsing.malware.enabled" = security.safeBrowsing.enable;
    "browser.safebrowsing.phishing.enabled" = security.safeBrowsing.enable;
    "browser.safebrowsing.downloads.enabled" = security.safeBrowsing.enable;
    "browser.safebrowsing.downloads.remote.enabled" =
      security.safeBrowsing.enable && security.safeBrowsing.remoteDownloads.enable;
    ...
    # OCSP was removed from Firefox (FF145). Certificate revocation is now
    # handled exclusively via CRLite (see security.pki.crlite_mode below).
    ...
    # Enable HTTPS-Only mode in all windows when requested.
    "dom.security.https_only_mode" = security.httpsOnly.enable;
    ...
    # Optionally exclude host candidates, which can break direct WebRTC connections.
    "media.peerconnection.ice.no_host" = security.webRTC.noHost.enable;
    ...
    # Disable using the OS theme accent color (Windows)
    ...
    # Keep WebGL's existing explicit compatibility toggle.
    "webgl.disabled" = cfg.misc.disableWebgl;

    # WebGPU is opt-in because it exposes additional graphics capability.
    "dom.webgpu.enabled" = security.webGPU.enable;
    ...
    # A supplied user agent override is appended below; otherwise Firefox uses its normal user agent.
    ...
    "datareporting.usage.uploadEnabled" = security.telemetry.enable;
    "datareporting.healthreport.service.enabled" = security.telemetry.enable;
    ...
    # RFP is opt-in; Strict ETP continues to provide Firefox fingerprinting protection by default.
    "privacy.resistFingerprinting" = security.resistFingerprinting.enable;
    ...
    "toolkit.telemetry.dap_enabled" = security.telemetry.enable;
    "toolkit.telemetry.user_characteristics_ping.opt-out" = !security.telemetry.enable;
    "toolkit.coverage.enabled" = security.telemetry.enable;
    "browser.newtabpage.activity-stream.telemetry.privatePing.enabled" = security.telemetry.enable;

    # Fingerprinting hardening
    "privacy.resistFingerprinting.randomization.daily_reset.enabled" = security.resistFingerprinting.enable;
    "privacy.fingerprintingProtection.remoteOverrides.enabled" = false;

    # Disable speculative preconnect (Phoenix)
    "network.preconnect" = false;

    # Obfuscate WebRTC host addresses (Phoenix)
    "media.peerconnection.ice.obfuscate_host_addresses" = true;

    # Block sensitive permissions by default (Betterfox / Phoenix)
    "permissions.default.geo" = 2;
    "permissions.default.desktop-notification" = 2;
    "permissions.default.xr" = 2;

    # Do not relax referrer policy across sites (Phoenix)
    "network.http.referer.disallowCrossSiteRelaxingDefault" = true;
  }
  // lib.optionalAttrs (security.userAgent != null) {
    "general.useragent.override" = security.userAgent;
  }
  // lib.optionalAttrs security.resistFingerprinting.enable {
    # Set RFP new window size max rounded values. Sizes round down in hundreds: width to 200s and
    # height to 100s, to fit the screen.
    # See: <https://bugzilla.mozilla.org/1330882>
    "privacy.window.maxInnerWidth" = 1600;
    "privacy.window.maxInnerHeight" = 900;

    # Dynamically resize the inner window by applying margins in stepped ranges when RFP is enabled.
    "privacy.resistFingerprinting.letterboxing" = security.resistFingerprinting.letterboxing.enable;
  }
